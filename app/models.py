...
class User(UserMixin, db.Model):
    ...
    def get_mail_verification_token(self, expires_in=600):
        return jwt.encode({'verify_mail': self.id, 'exp': time() + expires_in}, current_app.config['SECRET_KEY'], algorithm='HS256')
    ...
    @staticmethod
    def verify_mail_verification_token(token):
        try:
            id = jwt.decode(token, current_app.config['SECRET_KEY'], algorithms=['HS256'])['verify_mail']
        except:
            return
        return User.query.get(id)
